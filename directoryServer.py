...
class DirectoryServer(TCPServer):
    Get_info = "GET_SERVER: \nFILENAME: [a-zA-Z0-9_./]*\n\n"
    Get_rep = "PRIMARY_SERVER: %s\nPORT: %s\nFILENAME: %s%s\n\n"
    Get_info_slave = "GET_SLAVES: .*\nPORT: [0-9]*\n\n"
    Slave_header_rep = "SLAVES: %s\n\n"
    Slave_header = "\nSLAVE_SERVER: %s\nPORT: %s"
    Create_dir_info = "CREATE_DIR: \nDIRECTORY: [a-zA-Z0-9_./]*\n\n"
    Delete_dir_info = "DELETE_DIR: \nDIRECTORY: [a-zA-Z0-9_./]*\n\n"
    Database_loca = "Database/directories.db"
    ...
    def get_slaves(self, con, addr, text):
        ...
        return_string_com = self.Slave_header_rep % slave_string_com
    ...
    def find_host(self, path):
        ...
        con_req = db.connect(self.Database_loca)
        with con_req:
    ...
    def handler(self, message, con, addr):
        if re.match(self.Get_info, message):
            self.get_server(con, addr, message)
        elif re.match(self.Get_info_slave, message):
    ...
    def get_server(self, con, addr, text):
        ...
        return_string_info = self.Get_rep % (host, port, filename_info, slave_string_info)
    ...
    def remove_server(self, server):
        ...
        con_rem = db.connect(self.Database_loca)
        with con_rem:
    ...
    def create_tables(self):
        ...
        con_tab = db.connect(self.Database_loca)
        with con_tab:
